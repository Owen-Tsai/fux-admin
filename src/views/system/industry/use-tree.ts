import { getIndustryTree } from '@/api/system/industry'
import type { TreeProps } from 'tdesign-vue-next'

const useIndustryTree = () => {
  const filterText = ref('')
  const filter = ref<TreeProps['filter']>()
  const activeNodes = ref<number[]>([])

  const { data, pending, execute } = useRequest(getIndustryTree, { immediate: true })
...
export default useIndustryTree
